    ...
    <div className="flex min-h-screen flex-col bg-bridge-50">
      ...
      <header className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-bridge-500 text-white font-bold">
            TB
          </div>
          <div>
            <div className="font-bold text-bridge-900">TableBridge</div>
            <div className="text-xs text-slate-500">{user?.name} · <span className="capitalize">{user?.role}</span></div>
          </div>
        </div>
        <nav className="flex items-center gap-1">
          <NavItem to="/dashboard" icon={<LayoutDashboard size={16} />}>Dashboard</NavItem>
          <NavItem to="/kitchen" icon={<ChefHat size={16} />}>Kitchen</NavItem>
          <NavItem to="/history" icon={<HistoryIcon size={16} />}>History</NavItem>
          <NavItem to="/reports" icon={<BarChart3 size={16} />}>Reports</NavItem>
          <NavItem to="/settings" icon={<SettingsIcon size={16} />}>Settings</NavItem>
          <button
            onClick={logout}
            className="ml-2 flex items-center gap-1 rounded-lg px-3 py-1.5 text-sm font-semibold text-slate-500 hover:bg-slate-100"
          >
            <LogOut size={14} /> Sign out
          </button>
        </nav>
      </header>
      <main className="tablet-shell flex-1 p-4">{children}</main>
    </div>
  );
}
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        `flex items-center gap-1 rounded-lg px-3 py-1.5 text-sm font-semibold transition-colors ${
          isActive ? "bg-bridge-500 text-white" : "text-slate-600 hover:bg-slate-100"
        }`
      }
    >
      {icon} {children}
